...
const LastGames = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      axios.get(requests.lastGames + `&dates=2019-01-01,${todayDate}&search=${search}${platformSearch && `&platforms=${platformSearch}`}`)
    ])
  ...
  return (
    <main>
      <h1 className='text-6xl font-bold'>Last Games</h1>
        ...
        {games.map((game) => (
          <div key={game.id} className={`bg-${theme}-900 bg-opacity-15 rounded-xl overflow-hidden px-4 group mb-6`}>
            <Link to={`/game/${game.id}`} key={game.id}>
              ...
              <div className='absolute group-hover:-translate-y-8 opacity-0 max-w-72 group-hover:opacity-100 transition-all bg-neutral-950 py-1 px-4 rounded-xl'>
              ...
              <div className='flex justify-end flex-wrap gap-2 m-2 my-4'>
